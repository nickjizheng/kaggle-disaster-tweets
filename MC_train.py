...
# Multi-Channel Dataset that encodes text, keyword, and location separately
class MultiChannelTweetDataset(Dataset):
...
# Multi-Channel Model without auxiliary training
class MultiChannelElectraModel(nn.Module):
    def __init__(self, backbone: ElectraModel, dropout_probs=[0.1, 0.2, 0.3]):
        ...
        # Multi-sample dropout classifier (only main task)
    ...
    def forward(self, input_ids, attention_mask, kw_input_ids, kw_attention_mask,
                loc_input_ids, loc_attention_mask, labels=None):
        ...
        # Main classification only (no auxiliary)
        logits = self.classifier(features)
        
        if labels is not None:
            # Only main loss with label smoothing
...
# Create datasets
train_ds = MultiChannelTweetDataset(
...
val_ds = MultiChannelTweetDataset(
    val_df['text'].tolist(),
...
# Model and optimizer
model = MultiChannelElectraModel(backbone).to(device)
...
print(f"\nStarting multi-channel only training...")
    ...
    if best_thr_f1 > best_f1:
        ...
        if free_space > 1.0:
            try:
                ...
                path = f"{SAVE_DIR}/multichannel_only_f1_{best_thr_f1:.4f}.pth"
...
with open(f"{SAVE_DIR}/multichannel_only_results.json", "w") as f:
    json.dump(results, f)
