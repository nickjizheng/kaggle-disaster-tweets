...
# Combined Dataset with both multi-channel and hierarchical labels
class CombinedTweetDataset(Dataset):
    ...
    def _create_hierarchical_labels(self):
        """Create hierarchical labels (3 classes: no_kw, disaster_kw, other_kw)"""
        ...
        for i in range(len(self.keywords)):
            ...
            if kw == 'no_keyword':
                ...
                if is_disaster:
                    # Level 1: Disaster keyword
                    hier_labels.append(1)
                else:
                    # Level 2: Other keyword
    ...
    def __getitem__(self, idx):
        ...
        # Create enhanced text with metadata (main channel)
        metadata_parts = []
        if kw != 'no_keyword':
...
class CombinedElectraModel(nn.Module):
    def __init__(self, backbone: ElectraModel, dropout_probs=[0.1, 0.2, 0.3]):
        ...
        # Multi-head attention pooling for main text
        ...
        # Hierarchical head (3 classes: no_kw, disaster_kw, other_kw)
    ...
    def forward(self, input_ids, attention_mask, kw_input_ids, kw_attention_mask,
        ...
        # Multiple pooling strategies for main features
...
# Model and optimizer
model = CombinedElectraModel(backbone).to(device)
...
head_params = (list(model.kw_encoder.parameters()) + 
               ...
               list(model.shared_features.parameters()) + 
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch {epoch}"):
        ...
        loss, main_logits, hier_logits = model(
