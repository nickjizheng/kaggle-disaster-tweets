...
# Dataset with simplified hierarchical labels
...
class EnhancedHierarchicalElectraModel(nn.Module):
    def __init__(self, backbone: ElectraModel, dropout_probs=[0.1, 0.2, 0.3]):
        ...
        # Simplified hierarchical head (3 classes: no_kw, disaster_kw, other_kw)
    ...
    def forward(self, input_ids, attention_mask, labels=None, hier_labels=None):
        ...
        # Combine features
        combined = torch.cat([cls_emb, attn_emb, mean_emb], dim=-1)
        
        # Shared features
        shared_feats = self.shared_features(combined)
        ...
        if labels is not None:
            ...
            # Hierarchical loss (reduced weight)
            if hier_labels is not None:
                hier_loss = F.cross_entropy(hier_logits, hier_labels)
                # Reduce auxiliary weight to prevent interference
...
# Create datasets
train_ds = HierarchicalTweetDataset(
...
val_ds = HierarchicalTweetDataset(
    val_df['text'].tolist(),
...
# Model and optimizer
model = EnhancedHierarchicalElectraModel(backbone).to(device)
...
print(f"\nStarting enhanced hierarchical training...")
    ...
    if best_thr_f1 > best_f1:
        ...
        if free_space > 1.0:
            try:
                ...
                path = f"{SAVE_DIR}/enhanced_hierarchical_f1_{best_thr_f1:.4f}.pth"
...
with open(f"{SAVE_DIR}/enhanced_hierarchical_results.json", "w") as f:
    json.dump(results, f)