...
df_original = pd.read_csv('kaggle/train.csv', header=None, names=['id', 'keyword', 'location', 'text', 'target'])

# Load generated data from train_extension.csv if it exists
df_generated = None
if os.path.exists('kaggle/train_extension.csv'):
    print("Loading generated extension samples...")
    df_generated = pd.read_csv('kaggle/train_extension.csv', header=None, names=['id', 'keyword', 'location', 'text', 'target'])
    print(f"Found {len(df_generated)} generated samples")
else:
    print("No generated samples found in kaggle/train_extension.csv, using only original data")

print("Relabeling misidentified samples...")
df_original['target_relabeled'] = df_original['target']
for txt, lbl in MISLABELED_CORRECTIONS:
    df_original.loc[df_original['text'] == txt, 'target_relabeled'] = lbl

# Also apply relabeling to generated data if it exists
if df_generated is not None:
    df_generated['target_relabeled'] = df_generated['target']
    for txt, lbl in MISLABELED_CORRECTIONS:
        df_generated.loc[df_generated['text'] == txt, 'target_relabeled'] = lbl

print("Using original text without cleaning...")

# Process original data
df_original['keyword']  = df_original['keyword'].fillna('no_keyword')
df_original['location'] = df_original['location'].fillna('no_location')
df_original['keyword'] = df_original['keyword'].replace('', 'no_keyword')
df_original['location'] = df_original['location'].replace('', 'no_location')

# Process generated data if exists
if df_generated is not None:
    df_generated['keyword']  = df_generated['keyword'].fillna('no_keyword')
    df_generated['location'] = df_generated['location'].fillna('no_location')
    df_generated['keyword'] = df_generated['keyword'].replace('', 'no_keyword')
    df_generated['location'] = df_generated['location'].replace('', 'no_location')

print(f"\nOriginal data class distribution:")
print(f"Class 0 (Not Disaster): {sum(df_original['target_relabeled'] == 0)} ({100*sum(df_original['target_relabeled'] == 0)/len(df_original):.1f}%)")
print(f"Class 1 (Disaster): {sum(df_original['target_relabeled'] == 1)} ({100*sum(df_original['target_relabeled'] == 1)/len(df_original):.1f}%)")

if df_generated is not None:
    print(f"\nGenerated data class distribution:")
    print(f"Class 0 (Not Disaster): {sum(df_generated['target_relabeled'] == 0)} ({100*sum(df_generated['target_relabeled'] == 0)/len(df_generated):.1f}%)")
    print(f"Class 1 (Disaster): {sum(df_generated['target_relabeled'] == 1)} ({100*sum(df_generated['target_relabeled'] == 1)/len(df_generated):.1f}%)")

# COMBINED Dataset with both multi-channel inputs and hierarchical labels
class CombinedTweetDataset(Dataset):
    ...
    def _create_hierarchical_labels(self):
        ...
        for i in range(len(self.keywords)):
            ...
            if kw == 'no_keyword':
                # Level 1: No keyword (0)
                hier_labels.append(0)
            else:
                # Check if disaster keyword
                kw_lower = kw.lower()
                is_disaster = any(dk in kw_lower for dk in disaster_keywords)
                
                if is_disaster:
                    # Level 2: Disaster keyword (1)
                    hier_labels.append(1)
                else:
                    # Level 3: Other keyword (2)
    ...
    def __getitem__(self, idx):
        ...
        # Create enhanced text with metadata (from first script)
        metadata_parts = []
        if kw != 'no_keyword':
        ...
        # Main encoding (from first script)
        ...
        # Also encode keyword and location separately for auxiliary features (from first script)
...
# Multi-Sample Dropout (same as both scripts)
...
class CombinedMultiChannelHierarchicalModel(nn.Module):
    def __init__(self, backbone: ElectraModel, dropout_probs=[0.1, 0.2, 0.3]):
        ...
        # Lightweight encoders for auxiliary channels (from first script)
        ...
        # Multi-head attention pooling (from both scripts)
        ...
        # Cross-modal attention for keyword and location (from first script)
        ...
        # Feature fusion with gating (from first script)
        ...
        # Hierarchical head (from second script)
    ...
    def forward(self, input_ids, attention_mask, kw_input_ids, kw_attention_mask,
        ...
        # Process auxiliary channels (from first script)
        ...
        if labels is not None:
            ...
            # Hierarchical loss (reduced weight as in second script)
            if hier_labels is not None:
                hier_loss = F.cross_entropy(hier_logits, hier_labels)
                total_loss = main_loss + 0.1 * hier_loss  # Reduced weight from second script
            else:
...
# IMPORTANT: Split ONLY the original data for train/val
# Validation set contains ONLY original samples to maintain realistic difficulty
train_df_original, val_df = train_test_split(
    df_original, test_size=0.15, random_state=42, stratify=df_original['target_relabeled']
)

# Combine original training data with ALL generated samples for training
if df_generated is not None:
    print(f"\nCombining original training data with generated samples...")
    train_df = pd.concat([train_df_original, df_generated], ignore_index=True)
    print(f"Original training samples: {len(train_df_original)}")
    print(f"Generated samples added: {len(df_generated)}")
    print(f"Total training samples: {len(train_df)}")
else:
    train_df = train_df_original
    print(f"Training samples: {len(train_df)}")

print(f"Validation samples (original only): {len(val_df)}")

# Check final training distribution
print(f"\nFinal training set distribution:")
print(f"Class 0: {sum(train_df['target_relabeled'] == 0)} ({100*sum(train_df['target_relabeled'] == 0)/len(train_df):.1f}%)")
print(f"Class 1: {sum(train_df['target_relabeled'] == 1)} ({100*sum(train_df['target_relabeled'] == 1)/len(train_df):.1f}%)")

# Create datasets with COMBINED features
...
# Print hierarchical label distribution
print(f"\nHierarchical label distribution (Train):")
hier_counts = np.bincount([train_ds.hier_labels[i] for i in range(len(train_ds))])
print(f"No keyword: {hier_counts[0]} ({100*hier_counts[0]/len(train_ds):.1f}%)")
print(f"Disaster keyword: {hier_counts[1]} ({100*hier_counts[1]/len(train_ds):.1f}%)")
print(f"Other keyword: {hier_counts[2]} ({100*hier_counts[2]/len(train_ds):.1f}%)")
...
head_params = (list(model.kw_encoder.parameters()) + 
...
print(f"\nStarting COMBINED multi-channel + hierarchical training...")
    ...
    if best_thr_f1 > best_f1:
        ...
        if free_space > 1.0:
            try:
                ...
                path = f"{SAVE_DIR}/combined_multichannel_hierarchical_f1_{best_thr_f1:.4f}.pth"
...
with open(f"{SAVE_DIR}/combined_multichannel_hierarchical_results.json", "w") as f:
    json.dump(results, f)